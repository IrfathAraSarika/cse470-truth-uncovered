...
export async function findCase(caseId: string, userId: string, role: string) {
  const result = await pool.query(
    `select c.case_id, c.reference_no, c.report_id, r.reference_no as report_reference,
            c.status, c.opened_at, c.closed_at, c.resolution_notes, c.updated_at, r.title as report_title
     from cases c
     ...
     where (c.case_id::text = $1 or c.reference_no = upper($1) or r.report_id::text = $1 or r.reference_no = upper($1))
       and ($2 = 'admin' or citizen.user_id = $3)`,
    [caseId, role, userId],
  );
