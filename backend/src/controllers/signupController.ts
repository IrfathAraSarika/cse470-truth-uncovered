...
export async function signup(request: Request, response: Response, next: NextFunction) {
  ...
  if (normalizedRole !== 'citizen' && (typeof affiliation !== 'string' || affiliation.trim().length < 2)) { response.status(400).json({ error: 'Organization or department is required for this role.' }); return; }
  try { response.status(201).json({ userId: await createAccount(name.trim(), normalizedEmail, password, normalizedRole, typeof affiliation === 'string' ? affiliation.trim() : '') }); }
