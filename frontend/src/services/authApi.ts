...
export const signup = (name: string, email: string, password: string, role: 'citizen' | 'admin' = 'citizen') =>
  apiRequest<{ userId: string }>('/signup', { method: 'POST', body: JSON.stringify({ name, email, password, role }) });
