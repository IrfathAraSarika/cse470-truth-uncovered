    ...
    <div className="min-h-screen bg-bg-dark text-on-surface font-inter">
      ...
      <main className="max-w-[900px] mx-auto px-6 py-12">
        ...
        <h1 className="font-sora text-3xl font-bold text-white mt-2">
          Structured Case Lifecycle Tracker
        </h1>
        <div className="mt-7 flex flex-col sm:flex-row gap-3">
          <input
            ...
            className="flex-1 px-4 py-3 bg-black/40 border border-white/10 rounded-lg"
          />
          <button
            onClick={track}
        ...
        {caseRecord ? (
          <section className="mt-7 border border-white/10 rounded-lg bg-white/[0.02] overflow-hidden">
            <div className="p-6 border-b border-white/10 flex flex-col sm:flex-row justify-between gap-4">
              <div>
                <p className="text-xs uppercase font-bold text-on-surface/40">
                  Case Reference
                </p>
                <h2 className="font-sora text-base md:text-xl font-bold text-white mt-1 break-all">
                  {caseRecord.reference_no}
                </h2>
                <p className="text-xs text-brand-teal mt-2">
                  Report {caseRecord.report_reference}
                </p>
                <p className="text-sm text-on-surface/60 mt-2">
                  {caseRecord.report_title}
                </p>
              </div>
              <div>
                <p className="text-xs uppercase font-bold text-on-surface/40">
            ...
            <div className="p-6 md:p-8">
              {stages.map((stage, index) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`z-10 w-9 h-9 shrink-0 rounded-full grid place-items-center text-sm font-bold ${done ? "bg-brand-teal text-black" : "bg-white/5 border border-white/15 text-on-surface/40"}`}
                    >
                      {done ? "OK" : index + 1}
                    </div>
                    {index < stages.length - 1 && (
                      <div
                        className={`absolute left-[17px] top-9 w-px h-[calc(100%-36px)] ${done ? "bg-brand-teal/50" : "bg-white/10"}`}
                      />
                    )}
                    <div className="pt-1">
                      <h3
                        className={`font-sora font-bold ${done ? "text-white" : "text-on-surface/40"}`}
                      >
                        {label(stage)}
                      </h3>
          ...
          <section className="mt-7 border border-white/10 rounded-lg p-8 text-center bg-white/[0.02]">
            <p className="text-sm text-on-surface/50">
              Verified reports receive a case ID that can be tracked here.
            </p>
          </section>
        )}
      </main>
